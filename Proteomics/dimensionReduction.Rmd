...
Dimension reduction is a way to find patterns inside data. Usually two methods are most known, Singular Value Decomposition and Principal Components analysis, arriving to the same results. 
The aim is to find a set of uncorrelated variables that can explain as much of the observed variance as possible.

From another point of view, original data can be explained by another matrix made up of fewer variables, with more or less loss of details. This technique is common in image manipulation, especially in image compression. 
